...
function uploadImage() {
  ...
  var imagesRef = storageRef.child(
    "images/ads_" + Math.random().toString().substring(2, 6) + ".jpg"
  );
        ...
        .then((url) => {
          ...
          db.ref("All")
            ...
            .then((s) => {
              db.ref()
                .child(
                  `Advertisement/${adObject.category}/Adkey${Math.random()
                    .toString()
                    .substring(2, 6)}`
                )
                .set(s.key);
...
function showAllAds() {
  ...
  db.ref()
    ...
    .then((snapshot) => {
      ...
      <p class="card-text adsubmitby">Ad posted by: <b>Some User</b></p> 
    </div>`;
        })
        .join("");
      ...
      if (auth.currentUser == null) {
        ...
        db.ref()
          .child(`favourites/${auth.currentUser.uid}/`)
          .once("value")
          .then((snapshot) => {
...
function addToFav(adv) {
  if (auth.currentUser == null) {
    ...
    if (document.getElementById(adv).getAttribute("style") == "color: red;") {
      db.ref()
        .child(`favourites/${auth.currentUser.uid}/${adv}`)
        .remove()
        .then(() => {
      ...
      db.ref()
        .child(`favourites/${auth.currentUser.uid}/${adv}`)
        .set(Math.random().toString().substring(2, 6));
    }
  }
}
            ...
            for (let arr of dataArr) {
              db.ref()
                ...
                .then((snap) => {
                  ...
                  document.getElementById(
                    "addChartsFav"
                  ).innerHTML += `<div class="card card_style" id="${i}"  style="width: 18rem;">
                        <img class="card-img-top card_img" src="${val.url}" alt="image" style="background-color:white;">
                        <div class="card-body">
                          <h5 class="card-title">${val.cTitle}</h5>
                          <p class="card-text"><b>Description:</b> ${val.cDesc}</p>
...
function message() {
  if (screen.width <= 650) {
    ...
    document.getElementsByClassName("tab")[0].style.height = `${
      screen.height - 120
    }px`;
    document.getElementsByClassName("tabcontent")[0].style.height = `${
      screen.height - 120
    }px`;
  }
    ...
    .then((uid) => {
      ...
      mql.addListener((c) => {
        ...
        if (c.matches) {
          ...
          document
            .getElementsByClassName("tabcontent")[0]
        ...
        .then((snapshot) => {
          ...
          if (data == null) {
            ...
            for (val of arrVal) {
              ...
              document.getElementsByClassName(
                "tab"
              )[0].innerHTML += `<div class="tabChats" onclick="getMessages(event , '${info.reciverId}');">
...
function chat(val) {
  if (auth.currentUser == null) {
    ...
    if (adUser.userID == auth.currentUser.uid) {
      ...
      db.ref()
        .child(`chatRoom/${auth.currentUser.uid}/${adUser.userID}/info`)
        .set(infoObj)
        .then(() => {
          db.ref(`user/${auth.currentUser.uid}`)
            ...
            .then((snap) => {
              ...
              db.ref()
                .child(`chatRoom/${adUser.userID}/${auth.currentUser.uid}/info`)
                .set(infoObj2)
                .then(() => {
...
function getMessages(event, info) {
  ...
  if (btn.getAttribute("id") == null) {
    db.ref()
      .child(`chatRoom/${auth.currentUser.uid}/${info}/messages`)
...
function showChatInput(info) {
  document.getElementById(
    "chatInputText"
  ).innerHTML = `<input type="text" class="form-control" placeholder="Send a message" aria-label="Send a message" aria-describedby="button-addon2" id="chatInput">
...
function sendMessage(recId) {
  ...
  if (msg != "") {
    ...
    db.ref()
      .child(`chatRoom/${auth.currentUser.uid}/${recId}/messages`)
      .push(msgDetails);
    db.ref()
      .child(`chatRoom/${recId}/${auth.currentUser.uid}/messages`)
      .push(msgDetails);
    db.ref(`NotifityChat`)
      .push(msgDetails)
      .then(() => {
